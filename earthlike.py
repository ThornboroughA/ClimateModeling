...
model = exo.Model(workdir='model_run',  #names the folder where files will be kept while the model runs
                  modelname='model',    #sets the name for various other files and folders
...
model.configure(timestep=45,                                #length of model timestep in minutes
                ...
                #restarfile=,                                #path to a restart file from a previous model; delete if not used
                flux=1367,                                  #flux of sunlight at planet in W/m^2
                startemp=5780,                              #effective temperature of star in K
                year=360,                                   #length of year (orbital period) in 24-hour Earth days
             ...
             #   landmap=,                                   #path to .sra file holding land/sea mask; delete if not used
             #   topomap=,                                   #path to .sra file holding topography; delete if not used
                ...
                glaciers={'toggle': True,                   #toggles glacier module
                          ...
                          'initialh': -1                    #initial global glacier cover; -1 for no initial glaciers
                         }
               )
...
model.runtobalance(baseline=10,   #number of years necessary to hold balance for
                   maxyears=300,  #maximum number of years to run
                   minyears=20,   #minimum number of years to run
